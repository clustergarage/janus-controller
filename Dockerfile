...
WORKDIR /go/src/clustergarage.io/janus-controller/
# Put dependencies in their own layer so they are cached.
COPY go.mod go.sum ./
RUN go mod download
# Build the rest of the code.
COPY . /go/src/clustergarage.io/janus-controller/
RUN CGO_ENABLED=0 go build -a -installsuffix cgo -o januscontroller .
...
FROM alpine:edge
COPY --from=builder /go/src/clustergarage.io/janus-controller/januscontroller /
